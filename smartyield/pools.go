...
func (h *SmartYield) Pools(ctx *gin.Context) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&p.ProtocolId, &p.SmartYieldAddress, &p.ControllerAddress, &p.ModelAddress, &p.ProviderAddress, &p.OracleAddress, &p.JuniorBondAddress, &p.SeniorBondAddress, &p.CTokenAddress, &p.UnderlyingAddress, &p.UnderlyingSymbol, &p.UnderlyingDecimals, &p.RewardPoolAddress)
		if err != nil {
			response.Error(ctx, err)
			return
		}
